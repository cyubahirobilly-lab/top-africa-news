...
export default function ArticleForm({ isAdmin = false }) {
  ...
  return (
    <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <h2 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">
        {isAdmin ? 'Publish a new article' : 'Submit article for approval'}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5">
        ...
        <button type="submit" disabled={isSubmitting} className="btn-primary disabled:cursor-not-allowed disabled:opacity-70">
          {isSubmitting ? 'Saving…' : isAdmin ? 'Publish article' : 'Submit for admin review'}
        </button>
        {saved && (
          <p className="mt-3 text-sm text-green-600 dark:text-green-400">
            {isAdmin ? 'Article saved successfully.' : 'Your story has been submitted and is awaiting admin approval.'}
          </p>
        )}
